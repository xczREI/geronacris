...
// Father Name
$pdf->SetXY(15, 18.5);
fitTextInCell($pdf, 22, 21, 77, 5, $row['father_name']);
// Mother Name
$pdf->SetXY(115, 18.5);
fitTextInCell($pdf, 108, 21, 77, 5, $row['mother_name']);
// Child Name
$pdf->SetFont('Arial', '', 10);
$pdf->SetXY(109, 23);
...
// Birth Date
$pdf->SetXY(31, 27.5);
$date = date('F j, Y', strtotime($row['child_birth_date']));
$pdf->Cell(0, 10, strtoupper($date), 0, 1);
// Birth Place
$pdf->SetFont('Arial', '', 10);
$pdf->SetXY(93, 27.5);
fitTextInCellAddress($pdf, 93, 30, 100, 5, $row['birth_place']);
...
$pdf->SetXY(120, 49.5);
fitTextInCell($pdf, 120, 52, 64, 5, $row['mother_name']);
...
$sworn_day = addOrdinalSuffix((int)$row['sworn_day']);
...
// Sworn Mother Name
$pdf->SetXY(80  , 71);
fitTextInCell($pdf, 80, 73.4, 64, 5, $row['mother_name']);
$pdf->SetFont('Arial', '', 9.5);
// CTC
$pdf->SetXY(36, 76);
...
if ($row['late_birth_type'] == 'my birth') {
    ...
    $date2 = date('F j, Y', strtotime($row['late_birth_on']));
...
$pdf->SetXY(130.6, 244);
fitTextInCell($pdf, 130, 244, 29, 5, $row['applicant_than_owner']);
$pdf->SetFont('Arial', '', 9.5);
// Sign Day
$sign_day = addOrdinalSuffix((int)$row['sign_day']);
$pdf->SetXY(100, 265);
...
// Sign at
$pdf->SetXY(80, 273.5);
fitTextInCellAddress($pdf, 80, 273.5, 67, 4, $row['sign_at']);
// Affiant Name

$pdf->SetXY(120, 284);
$pdf->SetFont('Arial', '', 10);
fitTextInCell($pdf, 120, 284, 72, 7, $row['affiant_name']);
$pdf->SetFont('Arial', '', 9.5);
// Late Sworn Day

$late_sworn_day = addOrdinalSuffix((int)$row['late_sworn_day']);
$pdf->SetXY(97, 298);
