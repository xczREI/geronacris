...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reg_no'])) {
    ...
    // Table names to ensure rows exist (Using SELECT check instead of INSERT IGNORE to avoid duplicates)
    ...
    echo "<script>window.onload = function() { 
        alertify.alert('Save Report', '$report_body', function(){
            window.location.href = 'birth_cerf_edit_staff.php?reg_no=" . $reg_no . "';
        }); 
    }</script>";
}
                ...
                <div class="card-header bg-white pb-3 pt-4 border-bottom-0">
                    <div class="row align-items-center">
                        ...
                        <div class="col-md-9 d-flex justify-content-between">
                            ...
                            <button type="button" class="btn btn-light border py-1" style="background: white; width: 32%; text-align: center; text-decoration: none; color: black; font-size: 13px;" data-toggle="modal" data-target="#my1A">Print Form No. 1A</button>
                            <button type="button" class="btn btn-light border py-1" style="background: white; width: 32%; text-align: center; text-decoration: none; color: black; font-size: 13px;" data-toggle="modal" data-target="#my102">Print Form No. 102</button>
                        </div>
                    </div>
                </div>
...
function submitToPrint(targetUrl, modalId) {
    try {
        ...
        // 6. Hard-capture specific fields that might be missed by the generic loop
        // Attendant
        var attType = "";
